...
3. **When a new image is needed:** add a new row to `IMAGE_REQUESTS.md` using the schema in the media-prompting skill. Every manifest row is AI-generated — there is no photoshoot or stock-license path. The row MUST include Role, P-strategy (with verbatim quote from `design-direction.md`), Prompt, Negative prompt, Dimensions/aspect, Focal point, Alt intent, Rights/privacy notes, and Status `pending`. Then insert the marker `[NEEDS:image <that-ID>]` in the component at the slot. **Exception — specific real person or specific real place:** if the slot genuinely requires a named individual (e.g., the actual CEO for a leadership section) or the real premises (e.g., the actual headquarters exterior), do NOT write a manifest row. Use a content marker instead: `[NEEDS: client-supplied photo of <person/place> for <purpose>]`. The manifest is for generated, representative imagery — specific real-person/real-place assets are client-supplied content, not an image-generation request.
