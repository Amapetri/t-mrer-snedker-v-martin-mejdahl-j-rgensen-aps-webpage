...
- Does NOT auto-start the dev server or run browser-qa. The `/redesign` orchestrator owns the dev-server lifecycle (it starts `npm run dev` in the background during Step 6 and runs browser-qa across phase boundaries and the final review). If you somehow arrive at `/publish` with browser-qa missing — typically because `/redesign` failed to start the dev server, or because someone is running `/publish` standalone after editing files — the gate records `YELLOW` on check 2 but does NOT block by itself. Run `/review-browser` manually with `npm run dev` running, or re-run `/redesign` to fill the gap.
